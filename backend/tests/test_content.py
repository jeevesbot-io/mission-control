...
def test_get_pipeline_empty():
    ...
    with patch(
        "modules.content.service.ContentService.get_pipeline", new_callable=AsyncMock
    ) as mock:
        mock.return_value = ContentPipelineResponse(
            items=[], stats={"total": 0, "by_stage": {}, "by_type": {}}
        )
...
def test_get_pipeline_with_items():
    ...
    stats = {
        "total": 2,
        "by_stage": {"ideas": 1, "scripting": 1},
        "by_type": {"video": 1, "article": 1},
    }
    with patch(
        "modules.content.service.ContentService.get_pipeline", new_callable=AsyncMock
    ) as mock:
...
def test_create_content_item():
    ...
    with patch(
        "modules.content.service.ContentService.create_item", new_callable=AsyncMock
    ) as mock:
...
def test_update_content_item():
    ...
    updated = ContentItem(
        ...
        priority="high",
        created_at=datetime(2026, 2, 18),
        updated_at=datetime(2026, 2, 19),
    )
    with patch(
        "modules.content.service.ContentService.update_item", new_callable=AsyncMock
    ) as mock:
...
def test_delete_content_item():
    with patch(
        "modules.content.service.ContentService.delete_item", new_callable=AsyncMock
    ) as mock:
